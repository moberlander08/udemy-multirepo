...
docker image build -t moberlander08/multi-server:latest -t moberlander08/multi-server:$GIT_SHA -f ./server/Dockerfile ./server
docker image build -t moberlander08/multi-worker:latest -t moberlander08/multi-worker:$GIT_SHA -f ./worker/Dockerfile ./worker
